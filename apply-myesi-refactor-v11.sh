#!/bin/bash
set -e
echo "Applying round 11: switch to EmailJS (free, client-side, no server needed)..."

# The self-hosted mail server is no longer needed - EmailJS runs entirely from the browser
rm -rf "server"

mkdir -p "$(dirname "src/App.tsx")"
cat > "src/App.tsx" << 'R11FILEEOF_0'
/**
 * @license
...
cat > "src/components/About.tsx" << 'R11FILEEOF_1'
...
cat > "src/components/AboutContent.tsx" << 'R11FILEEOF_2'
import { motion } from 'motion/react';
...
cat > "src/components/AuditFeatures.tsx" << 'R11FILEEOF_3'
...
cat > "src/components/ContactHiring.tsx" << 'R11FILEEOF_4'
import { motion } from 'motion/react';
...
cat > "src/components/Footer.tsx" << 'R11FILEEOF_5'
...
cat > "src/components/FormToast.tsx" << 'R11FILEEOF_6'
...
cat > "src/components/Hero.tsx" << 'R11FILEEOF_7'
import { motion } from 'motion/react';
...
cat > "src/components/Navbar.tsx" << 'R11FILEEOF_8'
...
cat > "src/components/PCATeaser.tsx" << 'R11FILEEOF_9'
import { motion } from 'motion/react';
...
cat > "src/components/Packages.tsx" << 'R11FILEEOF_10'
import { motion } from 'motion/react';
import { Link } from 'react-router-dom';
...
cat > "src/components/PartnersSlider.tsx" << 'R11FILEEOF_11'
import { motion } from 'motion/react';
...
cat > "src/components/Product.tsx" << 'R11FILEEOF_12'
import { motion } from 'motion/react';
...
cat > "src/components/SupportSection.tsx" << 'R11FILEEOF_13'
import { useState } from 'react';
import { Link } from 'react-router-dom';
import { motion } from 'motion/react';
import {
...
cat > "src/components/TrustedBy.tsx" << 'R11FILEEOF_14'
import { motion } from 'motion/react';
...
cat > "src/data/complianceData.ts" << 'R11FILEEOF_15'
...
cat > "src/data/newsData.ts" << 'R11FILEEOF_16'
...
cat > "src/data/servicesData.ts" << 'R11FILEEOF_17'
...
cat > "src/data/trainingData.ts" << 'R11FILEEOF_18'
...
cat > "src/hooks/useSubmitForm.ts" << 'R11FILEEOF_19'
...
cat > "src/index.css" << 'R11FILEEOF_20'
...
cat > "src/lib/submitForm.ts" << 'R11FILEEOF_21'
...
// EmailJS's free tier includes 200 emails/month, which is plenty for a
// contact/lead form. You connect it to your own info@iseewaves.pk mailbox
// (or any mailbox) through their dashboard - EmailJS never sees or stores
// your mailbox password, only a service connection you authorize.
//
// SETUP REQUIRED (about 5 minutes):
// 1. Go to https://www.emailjs.com and create a free account.
// 2. Add an "Email Service" (Email Services tab) and connect it to
//    info@iseewaves.pk (Gmail, Outlook, or your own SMTP - EmailJS walks you
//    through whichever you have). Copy the Service ID it gives you.
// 3. Create an "Email Template" (Email Templates tab). A simple template like:
//      Subject: {{subject}}
//      From: {{name}} <{{email}}>
//      Body: {{message}}
//    works for most forms here - EmailJS will pass through whatever fields
//    each form collects (name, email, company, message, etc.) as template
//    variables automatically. Copy the Template ID.
// 4. Go to Account > General and copy your Public Key.
// 5. Paste all three values below.
// Until these are set, submissions will fail and show the "Something went
// wrong" toast rather than silently pretending to succeed.
...
export async function submitForm(data: Record<string, string>): Promise<boolean> {
  if (
    ...
    console.error(
      'EmailJS is not configured yet. Set EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, and EMAILJS_PUBLIC_KEY in src/lib/submitForm.ts (see the comments at the top of that file).'
    );
    return false;
  }
...
cat > "src/main.tsx" << 'R11FILEEOF_22'
...
cat > "src/pages/AboutPage.tsx" << 'R11FILEEOF_23'
...
cat > "src/pages/AllReportsPage.tsx" << 'R11FILEEOF_24'
import { motion } from 'motion/react';
...
cat > "src/pages/CareersPage.tsx" << 'R11FILEEOF_25'
import { motion } from 'motion/react';
...
cat > "src/pages/CompliancePage.tsx" << 'R11FILEEOF_26'
...
cat > "src/pages/ContactPage.tsx" << 'R11FILEEOF_27'
import { ArrowLeft } from 'lucide-react';
import { Link } from 'react-router-dom';
...
cat > "src/pages/CookiePolicyPage.tsx" << 'R11FILEEOF_28'
import { motion } from 'motion/react';
...
cat > "src/pages/DemoPage.tsx" << 'R11FILEEOF_29'
import { motion } from 'motion/react';
...
cat > "src/pages/LegalPage.tsx" << 'R11FILEEOF_30'
import { motion } from 'motion/react';
import { Shield } from 'lucide-react';
...
cat > "src/pages/MyesiLoginPage.tsx" << 'R11FILEEOF_31'
import { motion } from 'motion/react';
...
cat > "src/pages/PCAPage.tsx" << 'R11FILEEOF_32'
import { motion } from 'motion/react';
import {
  ArrowLeft,
  Users,
...
cat > "src/pages/PrivacyPolicyPage.tsx" << 'R11FILEEOF_33'
import { motion } from 'motion/react';
import { Shield } from 'lucide-react';
...
cat > "src/pages/ServiceDetailPage.tsx" << 'R11FILEEOF_34'
import { motion } from 'motion/react';
...
cat > "src/pages/ServicesPage.tsx" << 'R11FILEEOF_35'
import { motion } from 'motion/react';
...
cat > "src/pages/SimplePage.tsx" << 'R11FILEEOF_36'
import { motion } from 'motion/react';
...
cat > "src/pages/TermsOfServicePage.tsx" << 'R11FILEEOF_37'
import { motion } from 'motion/react';
import { Shield } from 'lucide-react';
...
cat > "src/pages/TrainingDetailPage.tsx" << 'R11FILEEOF_38'
import { motion } from 'motion/react';
import { ArrowLeft, Send, CheckCircle2, Loader2 } from 'lucide-react';
import { Link, useParams, Navigate } from 'react-router-dom';
...
cat > "src/pages/company/ExecutiveTeamPage.tsx" << 'R11FILEEOF_39'
import { motion } from 'motion/react';
...
cat > "src/pages/company/InvestorRelationsPage.tsx" << 'R11FILEEOF_40'
import { motion } from 'motion/react';
...
cat > "src/pages/partners/BecomePartnerPage.tsx" << 'R11FILEEOF_41'
...
cat > "src/pages/partners/ComparePlansPage.tsx" << 'R11FILEEOF_42'
import { motion } from 'motion/react';
...
cat > "src/pages/partners/FindPartnerPage.tsx" << 'R11FILEEOF_43'
...
cat > "src/pages/partners/PartnerPortalPage.tsx" << 'R11FILEEOF_44'
import { motion } from 'motion/react';
...
cat > "src/pages/partners/PartnerProgramsPage.tsx" << 'R11FILEEOF_45'
...
cat > "src/pages/resources/CommunityPage.tsx" << 'R11FILEEOF_46'
...
cat > "src/pages/resources/DocumentationPage.tsx" << 'R11FILEEOF_47'
...
cat > "src/pages/resources/TrainingPage.tsx" << 'R11FILEEOF_48'
import { motion } from 'motion/react';
...
cat > "src/pages/resources/TrustPage.tsx" << 'R11FILEEOF_49'
...
cat > "src/vite-env.d.ts" << 'R11FILEEOF_50'
...
echo ""
echo "NEXT STEPS:"
echo "1. Sign up free at https://www.emailjs.com"
echo "2. Add an Email Service connected to info@iseewaves.pk - copy the Service ID"
echo "3. Create an Email Template - copy the Template ID"
echo "4. Copy your Public Key from Account > General"
echo "5. Paste all three into src/lib/submitForm.ts"
echo "6. npm run build"
echo ""
echo "Full instructions are in the comments at the top of src/lib/submitForm.ts"