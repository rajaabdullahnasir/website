#!/bin/bash
set -e
echo "Applying round 10: self-hosted mail server (no third-party form service) replacing Formspree..."

mkdir -p "$(dirname "src/App.tsx")"
cat > "src/App.tsx" << 'R10FILEEOF_0'
/**
 * @license
...
cat > "src/components/About.tsx" << 'R10FILEEOF_1'
...
cat > "src/components/AboutContent.tsx" << 'R10FILEEOF_2'
import { motion } from 'motion/react';
          ...
          >
            <div className="flex flex-col md:flex-row gap-8 items-start">
              <div className="w-28 h-28 rounded-2xl bg-gradient-to-br from-teal-400/30 to-[#00C08B]/30 flex items-center justify-center shrink-0 border border-gray-200 mx-auto md:mx-0">
                <span className="text-3xl font-black text-[#0B2545]">AN</span>
              </div>
...
cat > "src/components/AuditFeatures.tsx" << 'R10FILEEOF_3'
...
cat > "src/components/ContactHiring.tsx" << 'R10FILEEOF_4'
import { motion } from 'motion/react';
...
cat > "src/components/Footer.tsx" << 'R10FILEEOF_5'
...
cat > "src/components/FormToast.tsx" << 'R10FILEEOF_6'
...
cat > "src/components/Hero.tsx" << 'R10FILEEOF_7'
import { motion } from 'motion/react';
...
cat > "src/components/Navbar.tsx" << 'R10FILEEOF_8'
...
cat > "src/components/PCATeaser.tsx" << 'R10FILEEOF_9'
import { motion } from 'motion/react';
...
cat > "src/components/Packages.tsx" << 'R10FILEEOF_10'
import { motion } from 'motion/react';
import { Link } from 'react-router-dom';
...
cat > "src/components/PartnersSlider.tsx" << 'R10FILEEOF_11'
import { motion } from 'motion/react';
...
cat > "src/components/Product.tsx" << 'R10FILEEOF_12'
import { motion } from 'motion/react';
...
cat > "src/components/SupportSection.tsx" << 'R10FILEEOF_13'
import { useState } from 'react';
import { Link } from 'react-router-dom';
import { motion } from 'motion/react';
import {
...
cat > "src/components/TrustedBy.tsx" << 'R10FILEEOF_14'
import { motion } from 'motion/react';
...
cat > "src/data/complianceData.ts" << 'R10FILEEOF_15'
...
cat > "src/data/newsData.ts" << 'R10FILEEOF_16'
...
cat > "src/data/servicesData.ts" << 'R10FILEEOF_17'
...
cat > "src/data/trainingData.ts" << 'R10FILEEOF_18'
...
cat > "src/hooks/useSubmitForm.ts" << 'R10FILEEOF_19'
...
cat > "src/index.css" << 'R10FILEEOF_20'
...
cat > "src/lib/submitForm.ts" << 'R10FILEEOF_21'
// All site forms submit through this single function instead of using mailto:,
// so submitting never opens the visitor's email app - it POSTs directly to
// YOUR OWN mail server (see /server in this project), which relays the
// message to info@iseewaves.pk by SMTP. No third-party form service
// (no Formspree, no EmailJS) is involved anywhere in this chain.
//
// SETUP REQUIRED:
// 1. Set up and run the server in the /server folder (see server/README.md) -
//    it needs your real SMTP credentials for info@iseewaves.pk.
// 2. Once that server is running somewhere reachable (a VPS, a subdomain,
//    etc.), create a .env file in this project's root with:
//      VITE_API_URL=https://api.iseewaves.pk
//    (or whatever address your server ends up running at).
// 3. Rebuild the site (npm run build) so Vite picks up the new env variable.
// Until VITE_API_URL points at a running server, submissions will fail and
// show the "Something went wrong" toast rather than pretending to succeed.

const API_BASE = import.meta.env.VITE_API_URL || 'http://localhost:4000';
export const FORM_ENDPOINT = `${API_BASE}/api/contact`;
...
export async function submitForm(data: Record<string, string>): Promise<boolean> {
  try {
    ...
    console.error('Form submission failed. Is the mail server running? See server/README.md', err);
    return false;
  }
}
R10FILEEOF_21
...
cat > "src/main.tsx" << 'R10FILEEOF_22'
...
cat > "src/pages/AboutPage.tsx" << 'R10FILEEOF_23'
...
cat > "src/pages/AllReportsPage.tsx" << 'R10FILEEOF_24'
import { motion } from 'motion/react';
...
cat > "src/pages/CareersPage.tsx" << 'R10FILEEOF_25'
import { motion } from 'motion/react';
...
cat > "src/pages/CompliancePage.tsx" << 'R10FILEEOF_26'
...
cat > "src/pages/ContactPage.tsx" << 'R10FILEEOF_27'
import { ArrowLeft } from 'lucide-react';
import { Link } from 'react-router-dom';
...
cat > "src/pages/CookiePolicyPage.tsx" << 'R10FILEEOF_28'
import { motion } from 'motion/react';
...
cat > "src/pages/DemoPage.tsx" << 'R10FILEEOF_29'
import { motion } from 'motion/react';
...
cat > "src/pages/LegalPage.tsx" << 'R10FILEEOF_30'
import { motion } from 'motion/react';
import { Shield } from 'lucide-react';
...
cat > "src/pages/MyesiLoginPage.tsx" << 'R10FILEEOF_31'
import { motion } from 'motion/react';
...
cat > "src/pages/PCAPage.tsx" << 'R10FILEEOF_32'
import { motion } from 'motion/react';
import {
  ArrowLeft,
  Users,
...
cat > "src/pages/PrivacyPolicyPage.tsx" << 'R10FILEEOF_33'
import { motion } from 'motion/react';
import { Shield } from 'lucide-react';
...
cat > "src/pages/ServiceDetailPage.tsx" << 'R10FILEEOF_34'
import { motion } from 'motion/react';
...
cat > "src/pages/ServicesPage.tsx" << 'R10FILEEOF_35'
import { motion } from 'motion/react';
...
cat > "src/pages/SimplePage.tsx" << 'R10FILEEOF_36'
import { motion } from 'motion/react';
...
cat > "src/pages/TermsOfServicePage.tsx" << 'R10FILEEOF_37'
import { motion } from 'motion/react';
import { Shield } from 'lucide-react';
...
cat > "src/pages/TrainingDetailPage.tsx" << 'R10FILEEOF_38'
import { motion } from 'motion/react';
import { ArrowLeft, Send, CheckCircle2, Loader2 } from 'lucide-react';
import { Link, useParams, Navigate } from 'react-router-dom';
...
cat > "src/pages/company/ExecutiveTeamPage.tsx" << 'R10FILEEOF_39'
import { motion } from 'motion/react';
...
cat > "src/pages/company/InvestorRelationsPage.tsx" << 'R10FILEEOF_40'
import { motion } from 'motion/react';
...
cat > "src/pages/partners/BecomePartnerPage.tsx" << 'R10FILEEOF_41'
...
cat > "src/pages/partners/ComparePlansPage.tsx" << 'R10FILEEOF_42'
import { motion } from 'motion/react';
...
cat > "src/pages/partners/FindPartnerPage.tsx" << 'R10FILEEOF_43'
...
cat > "src/pages/partners/PartnerPortalPage.tsx" << 'R10FILEEOF_44'
import { motion } from 'motion/react';
...
cat > "src/pages/partners/PartnerProgramsPage.tsx" << 'R10FILEEOF_45'
...
cat > "src/pages/resources/CommunityPage.tsx" << 'R10FILEEOF_46'
...
cat > "src/pages/resources/DocumentationPage.tsx" << 'R10FILEEOF_47'
...
cat > "src/pages/resources/TrainingPage.tsx" << 'R10FILEEOF_48'
import { motion } from 'motion/react';
...
cat > "src/pages/resources/TrustPage.tsx" << 'R10FILEEOF_49'
...
cat > "src/vite-env.d.ts" << 'R10FILEEOF_50'
/// <reference types="vite/client" />
R10FILEEOF_50

mkdir -p "$(dirname "server/.env.example")"
cat > "server/.env.example" << 'R10FILEEOF_51'
# Ask your email/domain host (wherever info@iseewaves.pk is hosted - your
# domain registrar, Zoho Mail, Google Workspace, cPanel, etc.) for these SMTP
# details. This is the same information you'd type into Outlook or Thunderbird
# to send mail from that account.

# Common examples:
#   Zoho Mail:            smtp.zoho.com          port 465
#   Google Workspace:     smtp.gmail.com          port 465 (requires an "app password", not your normal password)
#   cPanel / most hosts:  mail.yourdomain.com     port 465 or 587

SMTP_HOST=smtp.yourprovider.com
SMTP_PORT=465
SMTP_USER=info@iseewaves.pk
SMTP_PASS=your-mailbox-password-or-app-password

# Where submissions get delivered (defaults to info@iseewaves.pk if omitted)
MAIL_TO=info@iseewaves.pk

# Port this server listens on
PORT=4000
R10FILEEOF_51

mkdir -p "$(dirname "server/README.md")"
cat > "server/README.md" << 'R10FILEEOF_52'
# iSeeWaves Mail Server

A minimal Node.js server that sends the website's form submissions straight
to info@iseewaves.pk using your own mail account's SMTP credentials — no
Formspree, EmailJS, or any other third-party form service involved.

## How it works

1. Someone submits a form on the website (Contact, Careers, Trial, Service
   Request, Training Booking, etc.).
2. The website sends that data to this server (`POST /api/contact`).
3. This server logs into your mailbox via SMTP (the same protocol your email
   client uses) and sends the message to `info@iseewaves.pk`.
...
Open `.env` and fill in the real SMTP details for whoever hosts
`info@iseewaves.pk`'s mailbox (your domain/email provider — ask them for SMTP
host, port, username, and password if you don't already have them; it's the
same info you'd enter into Outlook or Thunderbird).
...
```
iSeeWaves mail server listening on port 4000
```
...
**Important:** this is a real server process, not a static file. It cannot
run on a static host like GitHub Pages. It needs somewhere that runs Node.js
continuously — a small VPS, a subdomain on your existing hosting if it
supports Node, or any always-on machine. Common free/cheap options: a small
DigitalOcean/Linode droplet, Railway, Render, or a spare VPS you already have.

Once it's running somewhere, keep it alive with a process manager, e.g.:

```bash
npm install -g pm2
pm2 start index.js --name iseewaves-mail
pm2 save
```

## Connecting the frontend to it

In the main project (not this `server/` folder), create a `.env` file with:

```
VITE_API_URL=https://api.iseewaves.pk
```

(or `http://your-server-ip:4000` if you haven't set up a domain for it yet).
Then rebuild the frontend (`npm run build`). The forms will now POST directly
to your server, which emails `info@iseewaves.pk` — no third party involved
anywhere in the chain.
R10FILEEOF_52

mkdir -p "$(dirname "server/index.js")"
cat > "server/index.js" << 'R10FILEEOF_53'
// Minimal mail relay for the iSeeWaves website forms.
//
// No third-party form service (no Formspree, no EmailJS). This uses YOUR OWN
// mail account's SMTP credentials to send email directly, the same way an
// email client would. All form submissions arrive at MAIL_TO (info@iseewaves.pk)
// via your own mail server.
//
// SETUP:
// 1. cd server
// 2. npm install
// 3. Copy .env.example to .env and fill in your real SMTP credentials
//    (ask whoever manages info@iseewaves.pk - your domain host, Zoho, Google
//    Workspace, cPanel, etc. - for the SMTP host/port/username/password).
// 4. npm start
// 5. This must run somewhere reachable by your website (a VPS, a small cloud
//    box, or any host that runs Node.js - NOT a static-only host like GitHub
//    Pages, which cannot run a server). Point VITE_API_URL (in the frontend's
//    .env) at wherever this ends up running, e.g. https://api.iseewaves.pk
...
const MAIL_TO = process.env.MAIL_TO || 'info@iseewaves.pk';
...
if (missingEnv.length === 0) {
  ...
  console.warn(
    `SMTP not configured - missing: ${missingEnv.join(', ')}. Set these in server/.env before forms will send.`
  );
}

app.get('/health', (req, res) => {
  res.json({ ok: true, smtpConfigured: !!transporter });
});
...
app.post('/api/contact', async (req, res) => {
  ...
  const data = req.body || {};
  const subject = data.subject || 'New website form submission';

  // Build a readable plain-text body from whatever fields the form sent.
  ...
  try {
    await transporter.sendMail({
      from: `"iSeeWaves Website" <${process.env.SMTP_USER}>`,
      to: MAIL_TO,
...
app.listen(PORT, () => {
  console.log(`iSeeWaves mail server listening on port ${PORT}`);
  if (!transporter) {
    console.log('Waiting on SMTP configuration in server/.env - see server/README.md');
  }
});
R10FILEEOF_53

mkdir -p "$(dirname "server/package-lock.json")"
cat > "server/package-lock.json" << 'R10FILEEOF_54'
{
  "name": "iseewaves-mail-server",
  "version": "1.0.0",
  "lockfileVersion": 3,
  "requires": true,
  "packages": {
...
cat > "server/package.json" << 'R10FILEEOF_55'
{
  "name": "iseewaves-mail-server",
  "version": "1.0.0",
  "description": "Minimal SMTP relay for iSeeWaves website forms. No third-party form service - sends directly via your own mail account.",
  "main": "index.js",
  "type": "commonjs",
  "scripts": {
    "start": "node index.js"
...
echo "2. Fill in real SMTP credentials for info@iseewaves.pk in server/.env"
echo "3. npm start   (inside server/) - this must run continuously somewhere reachable by your site"
echo "4. In the project root, create a .env file with VITE_API_URL=https://your-server-address"
echo "5. npm run build   (in the project root) to pick up the new env variable"
echo ""
echo "See server/README.md for full details."